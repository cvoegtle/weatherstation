...
class WeatherDataFetcher(private val pm: PersistenceManager, private val locationProperties: LocationProperties) {
  ...
  fun fetchSmoothedWeatherData(begin: Date, end: Date): List<SmoothedWeatherDataSet> {
  ...
  fun getLatestWeatherDataUnformatted(authorized: Boolean): UnformattedWeatherDTO {
    ...
    return UnformattedWeatherDTO(time = latest.timestamp!!,
                                 localTime = dateUtil.toLocalTime(latest.timestamp!!),
                                 temperature = latest.outsideTemperature!!,
                                 humidity = latest.outsideHumidity!!,
                                 ...
                                 rainLastHour = if (SmoothedWeatherDataSet.hasRainCounter(
                                     oneHourBefore)) calculateRain(latest.rainCounter!!,
                                                                   oneHourBefore!!.rainCounter!!) else null,
                                 rainToday = if (SmoothedWeatherDataSet.hasRainCounter(
                                     today) && WeatherDataSet.hasRainCounter(latest)) calculateRain(
                                     latest.rainCounter!!, today.rainCounter!!) else null)
  }
